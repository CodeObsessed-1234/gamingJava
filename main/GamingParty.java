...
public class GamingParty {
    ...
    {
        if(this.winner==null || this.lenPlayer()==0 || this.lenSnack()==0)
        {
        ...
        if(this.players.length==0)
            ...
            while ((int)hours > 0) {
                ...
                for (Player player : players) {
                    ...
                    if(snackCounter>snackIterator)
                        snackCounter = 0;
                    if (player!=null && snackCounter<=snackIterator) {
                        ++totalSnacks;
                        System.out.println(player.getName() + " food ->" + this.snacks[snackCounter].getDescription());
                    }
                    ++snackCounter;
                }
                --hours;
